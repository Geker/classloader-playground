...
public class LandClassLoader extends URLClassLoader {
	...
	/**
	 * Delegate config format, key is {@link DelegateType}, value is string list,
	 * item can be below case:
	 * <p/>
	 * <ol>
	 * <li>specify one class, content is a full qualified class name. example:
	 * {@code "com.foo.package1.Class1"},
	 * {@code "com.foo.package1.Class2$InnerClass2"}</li>
	 * <li>specify a package, content is a full qualified package name end by a dot.
	 * example: {@code "com.foo.package2.*"}</li>
	 * <li>specify a package and its sub packages, content is a full qualified
	 * package name end by two dot. example: {@code "com.foo.package3.**"}</li>
	 * </ol>
	 * <p/>
	...
	static DelegateType findDelegateTypeInDelegateConfigs(String className,
			Map<DelegateType, List<String>> delegateConfig, LandMatcher matcher) {
